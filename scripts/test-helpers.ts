...
expect.extend({
  ...
  toHaveProperty(received: unknown, path: string, expected?: unknown) {
    const value = path.split('.').reduce((v, k) => (v && typeof v === 'object' && k in v) ? (v as Record<string, unknown>)[k] : undefined, received as unknown);
